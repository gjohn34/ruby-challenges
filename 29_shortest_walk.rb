...
def check_north_south(walk_hash)
  if walk_hash["NORTH"] == walk_hash["SOUTH"]
    walk_hash["NORTH"] = 0
    walk_hash["SOUTH"] = 0
  elsif walk_hash["NORTH"] > walk_hash["SOUTH"]
    walk_hash["NORTH"] -= walk_hash["SOUTH"]
    walk_hash["SOUTH"] = 0
  elsif walk_hash["NORTH"] < walk_hash["SOUTH"]
    walk_hash["SOUTH"] -= walk_hash["NORTH"]
    walk_hash["NORTH"] = 0
  end
end

def check_east_west(walk_hash)
  if walk_hash["EAST"] == walk_hash["WEST"]
    walk_hash["EAST"] = 0
    walk_hash["WEST"] = 0
  elsif walk_hash["EAST"] > walk_hash["WEST"]
    walk_hash["EAST"] -= walk_hash["WEST"]
    walk_hash["WEST"] = 0
  elsif walk_hash["EAST"] < walk_hash["WEST"]
    walk_hash["WEST"] -= walk_hash["EAST"]
    walk_hash["EAST"] = 0
  end
end

def shortest_walk(walk)
  walk_hash = {"NORTH" => 0, "SOUTH" => 0, "EAST" => 0, "WEST" => 0}
  for direction in walk
    walk_hash[direction] += 1
  end
  check_north_south(walk_hash)
  check_east_west(walk_hash)
  walk_array = []
  for key, value in walk_hash
    if value != 0
    walk_array.push(key)
  end
  end
