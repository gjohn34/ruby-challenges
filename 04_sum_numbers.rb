...
def sum_numbers(numbers) # new method that takes 1 parameter
  ...
  for number in numbers #iterating through array
    if number.class.superclass != Numeric #if provided a non-number eg. string
      puts "Index #{numbers.index(number)} is not a number. Exluding #{number.class}."
      break
    else
      array_total += number #then adding that number to the total
    end
  end
  return array_total #returning the total. for result use puts/print
end
